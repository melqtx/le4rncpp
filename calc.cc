...
char getOperator() {
    ...
    cin >> op;
    while (!(op == '+' || op == '-' || op == '*' || op == '/' || op == '^' || op == 's' || op == 'c' || op == 't')) {
        cout << "Invalid operator. Please enter a valid operator (+, -, *, /, ^, sqrt, sin, cos, tan): ";
        cin >> op;
    }
    return op;
}
...
int main() {
    ...
    try {
        ...
        if (op == 's' || op == 'c' || op == 't') {
            num2 = getNumber();
            cout << "Result: " << performOperation(num1, num2, op) << endl;
        } else {
            cout << "Result: " << performOperation(num1, 0, op) << endl;
        }
    } catch (exception& e) {
